...
const bob = ['Hallo,', 'en', 'EN HALLO', 'Hallo, mijn vriend.', 'HALLO,'];
  ...
  if (Array.isArray(names)) {
    if (names.length === 2) {
      ...
      const formattedNames =
        names.slice(0, -1).join(', ') + ', ' + l[nameCase.and] + ' ' + names.slice(-1);
      return l[nameCase.hello] + ' ' + formattedNames + '.';
    }

    const uppercaseNames = names.filter(
      (name) => typeof name === 'string' && name === name.toUpperCase(),
    );
